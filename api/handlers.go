...
func CreateUser(w http.ResponseWriter, r *http.Request, p httprouter.Params)  {
	...
	su := &defs.SignedUp{Sucsess: true, SessionId: id}
...
func GetUserInfo(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	if resp, err:=json.Marshal(ui); err!= nil {
		...
		sendNormalResponse(w, string(resp), 200)
		// TODO
		log.Printf("return user ID : %s", resp)
	}
}
...
func ShowComments(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	// TODO 显示视频的 Comments
	log.Printf("List Comments. 1-")
	if !ValidateUser(w, r) {
		return
	}

	log.Printf("List Comments. 2- %s", utils.GetCurrentTimestampSec())
	vid := p.ByName("vid-id")
	cm, err := dbops.ListComments(vid, 0, utils.GetCurrentTimestampSec())
	log.Printf("List Comments. 3-")
	if err != nil {
